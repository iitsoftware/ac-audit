...
// Prepared statements
// authority_report_date steht hier — wie in jedem Statement darunter — an
// derselben Stelle wie in schema.sql: hinter authority_auditor, vor auditee.
const LINE_FIELDS = `id, audit_plan_id, sort_order, subject, regulations, location, planned_window, performed_date, signature,
  audit_no, audit_subject, audit_title, auditor_team, authority_auditor, authority_report_date, auditee,
...
const stmts = {
  ...
  // Schreibweg laufen, sonst wären sie zwar vorbelegt bzw. lesbar, aber kein
  // Aufrufer könnte sie je ändern. Weil das Statement die ganze Zeile ersetzt
  // und keine Oberfläche die zwei Felder heute rendert, reicht der Aufrufer den
  // gespeicherten Wert durch, statt ihn zu räumen.
  updateAuditPlanLine: db.prepare(
    `UPDATE audit_plan_line SET sort_order = ?, subject = ?, regulations = ?, location = ?, planned_window = ?, signature = ?,
      auditor_team = ?, authority_auditor = ?, authority_report_date = ?, auditee = ?,
      audit_start_date = ?, audit_end_date = ?, audit_location = ?,
  ...
  restoreAuditPlanLine: db.prepare(
    `INSERT INTO audit_plan_line (id, audit_plan_id, sort_order, subject, regulations, location, planned_window, performed_date, signature, audit_no, audit_subject, audit_title, auditor_team, authority_auditor, authority_report_date, auditee, audit_start_date, audit_end_date, audit_location, document_ref, document_iss_rev, document_rev_date, recommendation, audit_status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ),
