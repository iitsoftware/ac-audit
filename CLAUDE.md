            ...
            └── AuditPlanLine (id, audit_plan_id, subject, regulations, location, planned_window, audit metadata...)
                 └── AuditChecklistItem (id, audit_plan_line_id, section, regulation_ref, compliance_check, evaluation)
                      ...
                      └── CapItem (id, checklist_item_id, deadline, responsible_person, root_cause, corrective/preventive_action, completion_date, notified_at)
                           ...
                           ├── CapAction (id, cap_item_id, sort_order, kind, description, responsible_person, target_date, completion_date) — die n Maßnahmen einer Beanstandung
...
Maßnahmen-Block des Beanstandungs-Screens fährt auf diesen vier Routen (siehe
...
- **The word on the surface of a Behördenaudit is `Finding`, nowhere `Beanstandung`.** The record is one — the Finding of the Findingliste, its own screen, its 5-Why and its Maßnahmen — so a second word for it would cut the chain Besuch → Finding → 5-Why/Maßnahmen in half exactly where the list already stopped doing so. It is **labelling only**, in `public/companies.js` alone: no stored value, no route and no column changes, and internal audits keep `Eintrag` / `Bewertung` / `Compliance Check` verbatim. Covered are the nav segment and the `<h2>` (`findingSegmentName()`), the Stammdaten labels, the `openChecklistItemDialog()` labels and title, the heading `Erledigung des Findings` and every speaking empty state, which now reads `… — das Finding hat noch kein Level`. `Stufe` went with it: the Findingliste, the Audit-Line-PDF and the CM-003 all print that column as `Level`, so the detail screen calling it something else was the same split one level down
