...
- That same dialog carries a **document choice** (`#cap-export-doc-group` in `views/companies.ejs`, the `.radio-row` / `.radio-check` idiom of the CM-006 rating): **CM-003 Korrekturmaßnahme** (default, the previous behaviour) or **CM-002 Ursachenanalyse**, which goes to `GET /api/cap-items/:id/five-why/pdf`. It is a **Download**-only choice — there is no Versand of the CM-002, which the `.form-hint` under the group says out loud instead of leaving the user to discover it after sending. Its boundary is `selectedCapIds.length === 1` and **not** the screen: the CM-002 is an Einzelformular without a batch route, so the group is hidden on a Mehrfachauswahl out of *Offene Beanstandungen* and the download there stays exactly the batch route it was. The CAP level shares one entry as well and therefore offers it too — the route is deliberately ungated, a CAP without a `five_why` record yields the empty, hand-fillable form rather than a 404. All three entries (Übersicht, CAP-Ebene, Beanstandungs-Screen) now open the dialog through the one `openCapExportDialog(ids)`, which is where the resets live: E-Mail-Sektion zu and the choice back on CM-003 on every open, so a CM-002 picked once cannot leak into the next export
