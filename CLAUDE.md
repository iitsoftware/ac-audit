...
- The Stammdaten section of that screen (`#finding-basics`, filled by `renderFindingDetail()`) is the flat Findingliste turned upright — `Nr. | Referenz Paragraph | Beschreibung | Level | Frist` under the heading `Stammdaten` (the `<h2>` above already carries the name, so the section is not called `Finding` a second time) — and **auto-saves on blur / change** through `saveFindingFields()`, exactly like the report level (`saveLineFields()`) and the CAP level (`saveCapFields()`). `Nr.` is the vergebene `sort_order` and therefore a real, **editable** field (`fd-sort-order`, `type="number"`) — the screen on which the Nummer of a Finding is set; `Level` is the same `authorityEvalValues` menu labelled through `evalLabel(value, true)` as the dialog — `L3` included, so neither screen needs a fallback that re-adds a stored value the menu would otherwise collapse to `''`. `PUT /api/checklist-items/:id` **replaces** the whole row, so `section`, `auditor_comment` and `document_ref` travel unchanged from the loaded item — this screen does not show them and must not clear them; `sort_order` left that Durchreiche when the Nr. became a field, and an empty or non-integer input is rejected with a toast like an unreadable Frist rather than written through as `0`. Because the name of the screen **is** that number, `<h2>` and breadcrumb are patched right after the save (`renameFindingSegment()`) instead of waiting for the next load. The Frist goes out as `cap_deadline` (ISO), an empty field sends nothing and the deadline maintained on the CAP item stays (the same contract as the dialog), so an empty field is refilled from the stored value after saving instead of pretending the Frist was deleted
